    ...
    func validate(_ data: T) throws {
        if let min = self.min {
            guard data.count >= min else {
                throw ValidationError("is less than required minimum of \(elementDescription(count: min))")
            }
        }

        if let max = self.max {
            guard data.count <= max else {
                throw ValidationError("is greater than required maximum of \(elementDescription(count: max))")
            }
        }
    }
